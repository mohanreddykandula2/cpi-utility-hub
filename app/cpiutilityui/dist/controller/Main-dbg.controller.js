sap.ui.define([
  ...
  "sap/m/TextArea",
  "sap/m/Button"
], function (Controller, JSONModel, MessageToast, MessageBox, ValueState, Dialog, TextArea, Button) {
  "use strict";
  ...
  return Controller.extend("com.cpi.utility.cpiutilityui.controller.Main", {
    onInit: function () {
      this.getView().setModel(new JSONModel({
        ...
        sourceMode: "cpi",
        renameCount: 0,
        validationText: "No artifact loaded",
    ...
    onSourceModeChanged: function (event) {
    ...
    onDeployToCpi: function () {
      const commentInput = new TextArea({
        width: "100%",
        rows: 5,
        placeholder: "Example: Renamed script files for naming standard cleanup."
      });
      ...
      const dialog = new Dialog({
        ...
        beginButton: new Button({
          ...
          press: async () => {
            ...
            dialog.close();
            await this._deployToCpi(comment);
          }
        }),
    ...
    _deployToCpi: async function (comment) {
      const model = this.getView().getModel();
      const iflowId = String(model.getProperty("/iflowId") || "").trim();
    ...
    onScriptNameChanged: function (event) {
  const input = event.getSource();
  const context = input.getBindingContext();

  if (context) {
    context.getModel().setProperty(
      context.getPath() + "/newName",
  ...
  this._refreshGenerateState();
    ...
    _refreshGenerateState: function () {
      ...
      const renameCount = scripts.filter(script => {
        const nextName = String(script.newName || "").trim();
        return nextName && nextName !== script.originalName;
      }).length;
      ...
      model.setProperty(
        "/canDeployToCpi",
        hasRename && !hasError && Boolean(String(model.getProperty("/iflowId") || "").trim())
      );

      model.setProperty("/renameCount", renameCount);
      model.setProperty(
        "/validationText",
        hasError ? "Warnings need review" : scripts.length ? "Names look valid" : "No artifact loaded"
      );
      model.setProperty(
        "/validationState",
        hasError ? "Error" : scripts.length ? "Success" : "None"
      );
